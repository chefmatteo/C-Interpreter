...
void expression(int level) {
    ...
    {
        if (!token) {
            printf("%ld: unexpected token EOF of expression\n", line);
            exit(-1);
        }
        if (token == Num) {
            match(Num);
        ...
        while (token >= level) {
            ...
            if (token == Assign) {
                ...
                *++text = (expr_type == CHAR) ? LC : LI;
                ...
                printf("%ld: compiler error, token = %ld\n", line, token);
                exit(-1);
            }
        }
    }
...
void function_parameter() {
    ...
    while (token != ')') {
        ...
        // parameter name
        if (token != Id) {
            printf("%ld: bad parameter declaration\n", line);
            exit(-1);
        }
        if (current_id[Class] == Loc) {
            printf("%ld: duplicate parameter declaration\n", line);
            exit(-1);
        }
...
void function_body() {
    ...
    while (token == Int || token == Char) {
        ...
        while (token != ';') {
            ...
            if (token != Id) {
                // invalid declaration
                printf("%ld: bad local declaration\n", line);
                exit(-1);
            }
            if (current_id[Class] == Loc) {
                // identifier exists
                printf("%ld: duplicate local declaration\n", line);
                exit(-1);
            }
            match(Id);
...
void enum_declaration() {
    ...
    while (token != '}') {
        if (token != Id) {
            printf("%ld: bad enum identifier %ld\n", line, token);
            exit(-1);
        }
        next();
        if (token == Assign) {
            ...
            next();
            if (token != Num) {
                printf("%ld: bad enum initializer\n", line);
                exit(-1);
            }
            i = token_val;
            next();
...
void global_declaration() {
    ...
    while (token != ';' && token != '}') {
        ...
        if (token != Id) {
            // invalid declaration
            printf("%ld: bad global declaration\n", line);
            exit(-1);
        }
        if (current_id[Class]) {
            // identifier exists
            printf("%ld: duplicate global declaration\n", line);
            exit(-1);
        }
        match(Id);
...
int main(int argc, char **argv)
{
    ...
    if (!(text = old_text = malloc(poolsize))) {
        printf("could not malloc(%ld) for text area\n", poolsize);
        return -1;
    }
    if (!(data = malloc(poolsize))) {
        printf("could not malloc(%ld) for data area\n", poolsize);
        return -1;
    }
    if (!(stack = malloc(poolsize))) {
        printf("could not malloc(%ld) for stack area\n", poolsize);
        return -1;
    }
    if (!(symbols = malloc(poolsize))) {
        printf("could not malloc(%ld) for symbol table\n", poolsize);
        return -1;
    }
    ...
    if (!(src = old_src = malloc(poolsize))) {
        printf("could not malloc(%ld) for source area\n", poolsize);
        return -1;
    }
    // read the source file
